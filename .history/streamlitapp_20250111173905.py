...
with st.sidebar:
    ...
    with st.expander("⚙️ Refinement Settings", expanded=True):
        # Show disabled version when solving
    ...
    with solve_col1:
        if st.button("🚀 Solve", help="Start the simulation with current parameters", use_container_width=True):
    ...
    with solve_col2:
        if st.button("🔄 Reset", help="Reset the simulation", use_container_width=True):
            st.session_state.solving = False
            st.session_state.solutions = []
            st.session_state.marks = []
